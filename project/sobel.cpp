...
static void BENCH_SobelHardcodePowAndSqrtImplementationTrim(benchmark::State &state)
{
    // read in image as grayscale OpenCV Mat Object
    Mat input_image = imread(FILENAME, IMREAD_GRAYSCALE);
    ...
    for (auto _ : state)
    {
        ...
        for (int r = 0; r < n_rows; r++)
        {
            for (int c = 0; c < n_cols; c++)
            {
                float mag_x = padded_array[r][c] -
                              padded_array[r][c + 2] +
                              padded_array[r + 1][c] * 2 +
                              padded_array[r + 1][c + 2] * -2 +
                              padded_array[r + 2][c] -
                              padded_array[r + 2][c + 2];

                float mag_y = padded_array[r][c] +
                              padded_array[r][c + 1] * 2 +
                              padded_array[r][c + 2] -
                              padded_array[r + 2][c] +
                              padded_array[r + 2][c + 1] * -2 -
...
BENCHMARK(BENCH_SobelHardcodePowAndSqrtImplementationTrim)->DenseRange(0, DENSERANGEEND, 1);
