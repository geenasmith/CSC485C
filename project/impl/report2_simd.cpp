...
std::tuple<int32_t **, float **, int, int, int, int> preprocessing(std::string filename)
{
    Mat raw_image = imread(filename, IMREAD_GRAYSCALE);
    // convert image to CV_32F (equivalent to a float)
    Mat image;
    raw_image.convertTo(image, CV_32S);
    // gaussian filter to remove noise
    // GaussianBlur(image, image, Size(3, 3), 0, 0);

    auto right_pad_width = 6 - (image.rows % 6) + 1;

    // pad image with 1 px of 0s
    Mat padded_image;
    copyMakeBorder(image, padded_image, 1, 1, 1, right_pad_width, BORDER_CONSTANT, 0);

    int32_t **padded_array;
    padded_array = new int32_t *[padded_image.rows];
    padded_array[0] = new int32_t[padded_image.rows * padded_image.cols];
...
namespace openmp
{
std::string implementation = base + "_" + "simd";

void sobel_simd(int32_t **padded_array, float **output_array, int orig_n_rows, int orig_n_cols, int padded_n_rows, int padded_n_cols)
{
    ...
    for (int r = 0; r < orig_n_rows; r++)
    {
        for (int c = 0; c < orig_n_cols; c += 6)
        {
            ...
            for (int i = 1; i < 7; i++)
                output_array[r][c + i - 1] = g2[i];
        }
    }
}

void sobel_simd_openmpcoarse(int32_t **padded_array, float **output_array, int orig_n_rows, int orig_n_cols, int padded_n_rows, int padded_n_cols)
{
    const __m256i shift_left = _mm256_set_epi32(6, 5, 4, 3, 2, 1, 0, 0);
    const __m256i shift_right = _mm256_set_epi32(7, 7, 6, 5, 4, 3, 2, 1);
